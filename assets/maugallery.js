(function ($) {
  $.fn.mauGallery = function (options) {
    ...
    }, moveItemInRowWrapper(element) { element.appendTo(".gallery-items-row") }, responsiveImageItem(element) { if (element.prop("tagName") === "IMG") { element.addClass("img-fluid") } }, openLightBox(element, lightboxId) { $(`#${lightboxId}`).find(".lightboxImage").attr("src", element.attr("src")); $(`#${lightboxId}`).modal("toggle") }, prevImage(lightboxId) { const activeImageSrc = $(".lightboxImage").attr("src"); const imagesCollection = this.getFilteredImagesCollection(); const currentIndex = imagesCollection.findIndex((image) => image.attr("src") === activeImageSrc); const prevIndex = (currentIndex - 1 + imagesCollection.length) % imagesCollection.length; const prevImage = imagesCollection[prevIndex]; $(".lightboxImage").attr("src", prevImage.attr("src")) }, nextImage(lightboxId) { const activeImageSrc = $(".lightboxImage").attr("src"); const imagesCollection = this.getFilteredImagesCollection(); const currentIndex = imagesCollection.findIndex((image) => image.attr("src") === activeImageSrc); const nextIndex = (currentIndex + 1) % imagesCollection.length; const nextImage = imagesCollection[nextIndex]; $(".lightboxImage").attr("src", nextImage.attr("src")) }, getFilteredImagesCollection() {
      const activeTag = $(".tags-bar span.active-tag").data("images-toggle"); const imagesCollection = []; if (activeTag === "all") { $(".item-column img").each(function () { imagesCollection.push($(this)) }) } else { $(`.item-column img[data-gallery-tag="${activeTag}"]`).each(function () { imagesCollection.push($(this)) }) }
